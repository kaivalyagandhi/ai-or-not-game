...
export const AudioSystem = forwardRef<AudioControls, AudioSystemProps>(({
  ...
  // Audio file paths following naming convention
  ...
  useEffect(() => {
    const initializeAudio = async () => {
      ...
      try {
        ...
        const [backgroundMusic, clickSound, successSound, failureSound] = await Promise.all([
          loadAudioFile(audioConfig.backgroundMusic),
          loadAudioFile(audioConfig.clickSound),
          loadAudioFile(audioConfig.successSound),
          loadAudioFile(audioConfig.failureSound),
        ]);
        ...
        const totalFiles = Object.keys(audioConfig).filter(configKey => configKey !== 'enabled' && configKey !== 'volume').length;
    ...
    initializeAudio();
  }, [loadAudioFile, audioConfig.backgroundMusic, audioConfig.clickSound, audioConfig.successSound, audioConfig.failureSound, volume]);
  ...
  const handleMuteToggle = useCallback(() => {
    ...
    if (newMuted && backgroundMusicRef.current) {
  ...
  const playBackgroundMusic = useCallback(() => {
    if (muted || !backgroundMusicRef.current) return;
  ...
  const playClickSound = useCallback(() => {
    if (muted || !audioFiles.current.clickSound) return;
  ...
  const playSuccessSound = useCallback(() => {
    if (muted || !audioFiles.current.successSound) return;
  ...
  const playFailureSound = useCallback(() => {
    if (muted || !audioFiles.current.failureSound) return;
