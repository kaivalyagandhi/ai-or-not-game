...
} from '../leaderboard-manager-simple.js';
...
describe('Leaderboard Manager - Clear All', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should clear all leaderboards successfully', async () => {
    mockRedis.del.mockResolvedValue(1);

    await clearAllLeaderboards();

    // Should delete daily, weekly, and all-time leaderboards
    expect(mockRedis.del).toHaveBeenCalledTimes(3);
    expect(mockRedis.del).toHaveBeenCalledWith(expect.stringContaining('daily'));
    expect(mockRedis.del).toHaveBeenCalledWith(expect.stringContaining('weekly'));
    expect(mockRedis.del).toHaveBeenCalledWith(expect.stringContaining('all_time'));
  });

  it('should handle Redis errors gracefully', async () => {
    mockRedis.del.mockRejectedValue(new Error('Redis connection failed'));

    await expect(clearAllLeaderboards()).rejects.toThrow(LeaderboardError);
  });
});
